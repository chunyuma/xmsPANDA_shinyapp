...
runlogitreg<-function(X,Y,fdrmethod="BH",fdrthresh=0.05,pvalue.thresh=0.05){
     options(warn=-1)
    logistic_reg=TRUE
    res<-runlmreg(X=X,Y=Y,fdrmethod=fdrmethod,fdrthresh=fdrthresh,pvalue.thresh=pvalue.thresh,logistic_reg=TRUE)
     options(warn=0)
    return(res)
}


runlmreg<-function(X,Y,fdrmethod="BH",fdrthresh=0.05,pvalue.thresh=0.05,logistic_reg=FALSE){
     options(warn=-1)
    data_m_fc<-X[,-c(1:2)]
...
diffexp.biomarkers<-function(X=NA,Y=NA,feature_table_file=NA,class_labels_file=NA,feat.sel.methods=c("rf","rfe","pls","limma","lasso"),num.var.sel=10,iter.quantile.thresh=0.3,split.train.test=FALSE,train.pct=0.7,outloc,kfold=10,pca.ellipse=TRUE,Xtest=NA,Ytest=NA,rsdthresh=1,pls_vip_thresh=2,seedvalue=27611,learningsetmethod="CV",confounder.matrix=NA,confounderfdrmethod="BH",confounderfdrthresh=0.05,num.methods.sel=1,globalcor=FALSE,cor.method="spearman",networktype="complete",abs.cor.thresh=0.4,cor.fdrthresh=0.05,max.cor.num=100,net_node_colors=c("green","red"), net_legend=TRUE,niter=10,output.device.type="pdf",heatmap.col.opt="RdBu",boxplot.col.opt=c("white"),barplot.col.opt=c("grey57","grey90"),sample.col.opt="rainbow",mz.thresh=1,time.thresh=10,svm_kernel="radial",good_feats_index=NA,pvalue.thresh=0.05,plots.width=8,plots.height=8,plots.res=600, plots.type="cairo",num_nodes=2,ylabel="Intensity",cex.plots=0.7,tune_classifiers=FALSE,find.common.features=FALSE)
{
...
if(tune_classifiers==TRUE)
{
    if(length(good_feats_index)>=3)
    {
        ...
        set.seed(seedvalue)
        tune_plslda <- suppressWarnings(CMA::tune(X = X, y = Y, learningsets = fiveCV10iter, classifier = pls_ldaCMA, grids = list(comp = 1:5),trace=FALSE))
        
   
        set.seed(seedvalue)
        tune_plsrf <- CMA::tune(X = X, y = Y, learningsets = fiveCV10iter, classifier = pls_rfCMA, grids = list( ),trace=FALSE)
    }
        ...
        if(FALSE){
            if(dim(X)[2]>100){
                tune_nnet <- CMA::tune(X = X, y = Y, learningsets = fiveCV10iter, classifier = nnetCMA, grids = list(size=1:3,decay=c(0.0001,0.001, 0.01,0.1)), trace=FALSE)
            }else{
                 tune_nnet <- CMA::tune(X = X, y = Y, learningsets = fiveCV10iter, classifier = nnetCMA, grids = list(size=1:3,decay=c(0.0001,0.001, 0.01,0.1)), trace=FALSE)
    ...
    set.seed(seedvalue)
    class_nnet<- classification(X =X, y = Y, learningsets = fiveCV10iter, classifier = nnetCMA,trace=FALSE,size=3,decay=0.1) #tuneres =tune_nnet,
    set.seed(seedvalue)
    ...
    save(class_scda,eval_scda,eval_measure,X,Y,fiveCV10iter,file="scda_debug1.Rda")
...
if(is.na(Xtest)==FALSE){
    ...
    if(tune_classifiers==TRUE){
        ...
        #if(best_classifier_name==classifier_names[4])
        {
            ...
            set.seed(seedvalue)
            class_res2<-svmCMA(X = X, y = Y, learnind = learnmatrix, cost = b1[[1]]$cost,gamma=b1[[1]]$gamma,probability=TRUE)
            }else{
        ...
        #if(best_classifier_name==classifier_names[6])
        {
            ...
            class_res2 <- nnetCMA(X =X, y = Y, learnind=as.numeric(learnmatrix),size=3,decay=0.1) #size=b1[[1]]$size,decay=b1[[1]]$decay
        ...
        print(legend(par('usr')[2], par('usr')[4], bty='n', xpd=NA,c("Training (kfold)","Test"), pch = rep(19,2), pt.cex = 0.6, title = "",cex=0.7,col=barplot.col.opt))
  ...
  # barplot(acc_mat,xaxt="n",main=mainlab,col=barplot.col.opt,ylab="Classification accuracy(%)",ylim=c(50,100),xpd=FALSE)
    ...
    print(legend(par('usr')[2], par('usr')[4], bty='n', xpd=NA,c("Training (kfold)","Training (permuted)"), pch = rep(19,2), pt.cex = 0.6, title = "",cex=0.7,col=barplot.col.opt))
...
get_fcs<-function(kegg_species_code="hsa",database="pathway",metab_data,reference_set=NA,type.statistic="pvalue"){
...
get_roc<-function(dataA,classlabels,classifier="svm",kname="radial",rocfeatlist=seq(2,10,1),rocfeatincrement=TRUE,testset=NA,testclasslabels=NA,mainlabel=NA,col_lab=NA,legend=TRUE,newdevice=FALSE,mz_names=NA){
   ...
   if(is.na(mainlabel)==TRUE){
    ...
    options(warn=0)
    return(list("auc"=p1@y.values[[1]],"roc"=pred1))
}
